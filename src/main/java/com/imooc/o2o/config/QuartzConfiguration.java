...
public class QuartzConfiguration {
     ...
     * 需要注入ProductSellDailyService
     * @return
     */
    @Bean("jobDetailFacotry")
    public MethodInvokingJobDetailFactoryBean createJobDetail(ProductSellDailyService productSellDailyService) {
     ...
     * 需要注入jobDetailFactory
     * @return
     */
    @Bean("productSellDailyTriggerFactory")
    public CronTriggerFactoryBean createProductSellDailyTrigger(MethodInvokingJobDetailFactoryBean jobDetailFactory) {
        ...
        // 设定cron表达式
        triggerFactory.setCronExpression("0 0 0 * * ? *");//每天凌晨0点钟执行一次
		//triggerFactory.setCronExpression("0/3 * * * * ? *");//每三秒执行一次
        return triggerFactory;
    }

    /**
     * 创建调度工厂并返回
     *需要注入productSellDailyTriggerFactory
     * @return
     */
    @Bean("schedulerFactory")
    public SchedulerFactoryBean createSchedulerFactory(CronTriggerFactoryBean productSellDailyTriggerFactory) {
